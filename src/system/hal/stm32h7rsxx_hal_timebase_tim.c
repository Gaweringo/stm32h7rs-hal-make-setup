...
HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority) {
    ...
    uwPrescalerValue = __HAL_TIM_CALC_PSC(uwTimclock, TIM_CNT_FREQ);
    ...
    */
    //   or originally: ((uint32_t)uwTickFreq  * (TIM_CNT_FREQ / TIM_FREQ)) - 1U;
    htim6.Init.Period = __HAL_TIM_CALC_PERIOD(uwTimclock, uwPrescalerValue, TIM_FREQ);
...
void TIM6_IRQHandler(void) {
    // This will result in HAL_TIM_PeriodElapsedCallback being called
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    // TIM6 is our hal tick timer, because threadX uses systick
